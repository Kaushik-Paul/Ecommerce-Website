...
export class ProductService {
  ...
  getProduct(theProductId: number):Observable<Product> {
    ...
    //ned to build the URL based on the product id
  ...
  getProductListPaginate(thePage: number, thePageSize: number,
    ...
    //need to build URL based on category id, page and the page size
    const searchUrl = `${this.baseUrl}/search/findByCategoryId?id=${theCategoryId}`
  ...
  searchProductPaginate(thePage: number, thePageSize: number,
    ...
    //need to build URL based on keyword, page and the page size
    const searchUrl = `${this.baseUrl}/search/findByNameContaining?name=${theKeyword}`
                        + `&page=${thePage}&size=${thePageSize}`;
